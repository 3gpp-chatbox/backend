...
this section is named: {section_name}, and it is mainly about one procedure, analyze text  and fine that main procedure, and then focus on that procedure,extract the procedure information  and use below pattern to mapping it (below is an example) finally,also Return description, key information ,also let me know the procedure name.
...
procedural_info = process_section("5.5.1.2.2", "section_content_multiple_paragraphs.db")

if procedural_info:
    save_procedural_info_to_txt(procedural_info, "0225-prompt1.txt")
else:

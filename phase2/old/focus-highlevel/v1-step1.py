...
Your task is to extract a **State-Event Transition Graph** from the 3GPP procedure: "{section_name}". This graph captures the high-level control flow of the procedure — not all low-level or internal protocol steps. Only include transitions that represent major state changes or clearly defined message-driven control logic.
...
Before building the final graph, analyze the provided text step-by-step and extract:

- Focus on **core system states and transitions** that drive the overall logic.
- **Do not include** minor message acknowledgments, retries, or sub-phases unless they are critical to the main flow.
- Use your domain understanding to judge what constitutes a **procedural milestone**, not every interaction.
- Extract:
- All **possible States**: Quote the exact phrase or rephrase only if meaning is clear
- All **Events**: Capture all explicit message exchanges or triggering actions mentioned in the text
- Candidate **Transitions**: Where a state leads to another based on event


**Non-example** (do not include unless critical):
- Event: `Auth_Challenge_Resent` (unless it creates a distinct state or causes a new transition)
- State: `MME_CheckingIntegrity` (unless the overall flow logic hinges on it)
...
If a transition is strongly implied through temporal or causal cues (e.g., “then”, “after that”), and it reflects a change in logical phase, you may include it — but only if both the state and event are grounded in the text.
If relevant, prefix nodes with entity names (e.g., 'UE_Attaching', 'MME_Waiting') to disambiguate roles.

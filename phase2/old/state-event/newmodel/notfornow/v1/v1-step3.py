...
def extract_procedural_info(section_name, extracted_data, original_content):
    ...
    You are a 3GPP metadata annotator,You are given **two parts of context** for procedure "{section_name}", 
    1. **First part**:**State-Event-State transitions** extracted previously for the procedure "{section_name}".
    2. **Second part**: The **original section of the 3GPP specification document** that describes the procedure.
    your task is to Enhance this procedure transitions with the following:

- **Conditions** (e.g., "Authentication successful")
- **Parameters** (e.g., IMSI, TAI)
- **Metadata** (e.g., message_type, timestamps)

Use this example format:
{
  "transition": "(UE_Authenticating, Authentication_Challenge) → UE_Securing",
...
**Strict Rule**:  
- When identifying or extracting information, **you must rely solely on the provided content** (first part and second part).
- Do **not** make assumptions, and do **not** introduce information beyond the provided sections.

First Part:
**State-Event-State transitions** extracted previously for the procedure "{section_name}".:

...
def extract_procedural_info(section_name, extracted_data, original_content):
    ...
    prompt = f"""
You are a 3GPP procedure analysis expert. You are given **two parts of context** for procedure "{section_name}":

1. **First part**: The **states**, **events**, and **actions** extracted previously for the procedure "{section_name}".
2. **Second part**: The **original section of the 3GPP specification document** that describes the procedure.

Your task:
- Using the **states**, **events**, and **actions** from the first part, and the original content from the second part (the specification), identify and create a list of **State-Event-State transitions**.
- For each transition, include the **action** that causes the transition, and ensure that you maintain the relationships between states, events, and actions.
...
When extracting information, you must rely solely on the provided content (first part and second part).
...
First Part:
States, Events, and Actions of the procedure "{section_name}" (Extracted previously):
...
def process_procedure(section_name):
    ...
    extracted_data = read_json_file("v1-step1-nodescription.json")
...
if procedural_info:
    save_to_txt(procedural_info, "v1-step2.txt")
else:

...
1. **First Part**: The high-level flow property graph components extracted previously for the procedure "{section_name}" (states, events, and transitions) **without descriptions**.
2. **Second Part**: The original section of the 3GPP specification document that describes the procedure in detail.

Your task:
- Using the **states**, **events**, and **transitions** from the first part, and the detailed content from the second part (the 3GPP specification), **enrich** the transitions with the following:
  - **Descriptions** of what causes the transition (e.g., triggers or conditions).
  - **Conditions** that apply to the transition (e.g., successful authentication for a state change).

**Strict Rule**:
When identifying information, you may rely on your pretrained knowledge of 3GPP procedures to help understand terms like states, events, and actions if they are not explicitly defined in the provided content.
...
if procedural_info:
    save_to_txt(procedural_info, "v1-step2.json")
else:

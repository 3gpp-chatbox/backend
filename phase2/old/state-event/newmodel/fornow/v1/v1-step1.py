...
def extract_procedural_info_from_text(section_name, text):
    prompt = f"""
You are a 3GPP procedure analysis expert. Your task is to extract high-level flow property graph components from the procedure "{section_name}". 
You will extract the following for the procedure  "{section_name}" from my provided 3GPP specification section context:

1. **States**: Identifiable system or UE conditions (e.g., UE_Powered_On, UE_Attaching).
2. **Events**: Discrete actions or messages that trigger transitions (e.g., Attach_Request_Received).
3. **Transitions**: Basic transitions between states, triggered by events (e.g., UE_Powered_On → Attach_Request_Received → UE_Attaching).

**Strict Rule**: 
When identifying information, you may rely on your pretrained knowledge of 3GPP procedures to understand terms like states, events if they are not explicitly defined in the provided content.
...
Ensure your output is structured as follows:
...
{{
  ...
  "graph": {{
    "nodes": [
      {{"id": "UE_Powered_On", "type": "state"}},
      {{"id": "Attach_Request_Received", "type": "event"}},
      {{"id": "UE_Attaching", "type": "state"}},
      {{"id": "Authentication_Challenge", "type": "event"}},
      {{"id": "UE_Authenticating", "type": "state"}}
    ],
    "edges": [
      {{
        "from": "UE_Powered_On",

...
def extract_procedural_info_from_text(section_name, text):
 ...
 remove below json's "label"field values,also "label" this attribute.

 for example:
 make this:
 {{
        "from": "Event_Transmission_Failure_REG_COMPLETE",
        "to": "UE_5GMM_REGISTERED",
        "action": "Retry_REGISTRATION_COMPLETE",
        "properties": {{
            "actions": ["Resend REGISTRATION COMPLETE (Implementation specific retry)"]
        }},
        "condition": "Current TAI has not changed",
        "section_reference": "5.5.1.2.7.j",
        "label": "Retry REG COMPLETE /n TAI Unchanged"
    }}
    to this:
    {{
        "from": "Event_Transmission_Failure_REG_COMPLETE",
...
input_file_path = "step1.json"  # Path to your input text file
...
if procedural_info:
    save_procedural_info_to_json(procedural_info, "step1-nolabel.json")
else:
    print("Failed to extract procedural information")

if save_procedural_info_to_json:
   clean_json("step1-nolabel.json")
else:

...
def extract_procedural_info(section_name, extracted_data, original_content):
    ...
    prompt = f"""
Here is a list of extracted explicitly states from the 3gpp procedure "{section_name}"on the UE side. Validate whether any states are missing, inaccurate. what i want is exlicitly states involved in procedure "{section_name}" on UE side, so check the extracted result crefully and correct it,
Your answer must be fully based on the provided text, with no assumptions or inferred .
correct it and only return corrected list.

...
def extract_procedural_info(section_name, extracted_data, evaluation):
    ...
    prompt = f"""
Based on the validation results and the original extracted transition, correct it and return same format
only return json format

original extracted transition json:
{extracted_data}
...
def process_procedure(section_name):
    ...
    extracted_data = read_json_file("step2b-correct.json")
    evaluation = read_text_file("step2c-validatecontent.txt")
...
if procedural_info:
    save_to_json(procedural_info, "step2d-correctcontent.json")
else:

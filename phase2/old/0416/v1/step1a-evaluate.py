...
def extract_procedural_info(section_name, extracted_data, original_content):
    ...
    prompt = f"""
Here is a list of extracted states(explicity and implicity all included) from the 3gpp procedure "{section_name}"on the UE side. Validate whether any states are missing, inaccurate, or not grounded in the original text provided below.
Identify each issue clearly.
Your answer must be fully based on the provided text, with no assumptions or inferred .
...
if procedural_info:
    save_to_txt(procedural_info, "step1a-evaluate.txt")
else:

...
def extract_procedural_info(section_name, extracted_data, originaltext):
    ...
    prompt = f"""
Using the following list of states and the original 3GPP spec text, extract all state-to-state transitions(no matter trigger by event or condition or both or what) that are explicitly described in the text for the procedure "{section_name}"
...
excerpt (the full sentence or paragraph describing the transition)
Extract every transition, even if multiple go between the same pair of states.
Format:

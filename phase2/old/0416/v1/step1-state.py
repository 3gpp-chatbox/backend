...
def extract_procedural_info_from_text(section_name, text):
    prompt = f"""
Extract all the states involved in the procedure {section_name} on the UE side from the following section of the 3GPP specification.
Include all types of states, no matter how small or transient.Include both:
Explicitly named states mentioned in the text (clearly labeled).
Implied states if they represent a significant phase between two actions and are grounded in the text (label them as 'inferred').
Your answer must be fully based on the provided text, with no assumptions or inferred states.
Output a list of state names.

provided text below,which is sections(also subsections) content about this procedure"{section_name}" from specification :
...
if procedural_info:
    save_procedural_info_to_json(procedural_info, "step1-state.txt")
else:
    print("Failed to extract procedural information")

if save_procedural_info_to_json:
   clean_json("step1-state.txt")
else:

...
def extract_procedural_info(section_name, extracted_data, evaluation):
    ...
    prompt = f"""
Based on the validation results and the original text, provide a corrected list of all the states for procedure{section_name} on the UE side.
only return list of state name
...
def process_procedure(section_name):
    ...
    evaluation = read_text_file("step1a-evaluate.txt")
...
if procedural_info:
    save_to_json(procedural_info, "step1b-correct.txt")
else:

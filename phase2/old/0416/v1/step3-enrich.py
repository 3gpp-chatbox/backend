...
def extract_procedural_info_from_text(section_name, text):
    prompt = f"""
one by one For each transition below, extract the following metadata if available in the supporting excerpt:
event (what triggers it)
condition (any conditional logic)
action:what the UE does (the behavior or activity it performs) during or as a result of the transition — excluding the actual state change.

Use only what’s clearly stated in the original excerpt. If a value is missing, leave it blank or null.
...
{{
  ...
  "event": "MSG_X received",
  "condition": null,
  "action":"starts timer t3510
}}
...
if procedural_info:
    save_procedural_info_to_json(procedural_info, "step3-enrich.json")
else:
    print("Failed to extract procedural information")

if save_procedural_info_to_json:
   clean_json("step3-enrich.json")
else:

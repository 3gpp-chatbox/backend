...
- Ensure each node is either a **UE-visible event** or an **explicitly named UE state**.
  - **State** examples: "5GMM-REGISTERED", "DEREGISTERED.LIMITED-SERVICE"
  - **Event** examples: "Receive REGISTRATION REJECT", "T3510 expires"
- Confirm that **events** describe something that happens **to** the UE (not something the UE does).
- Confirm that **actions performed by the UE** (e.g., "Send X", "Store context") are **not included** as nodes.
...
how to identify missing event:Events: UE-visible triggers that cause a transition in the flow. events are always things that happen to the UE, not things the UE does.like timer expiries or received messages (e.g., `"Receive REGISTRATION ACCEPT"`, `"T3510 expires"`).If "Reject" refers to something the UE receives, treat it as an event.
how to identify missing state: explicitly named UE states from the spec (e.g., `"5GMM-DEREGISTERED"`).

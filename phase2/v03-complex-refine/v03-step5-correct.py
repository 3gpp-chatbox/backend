...
Extracted property graph JSON
An evaluation list of issues with specific corrections

Your job:
Only modify the parts listed in the "issues" array.
Do not add new nodes or edges unless specified.
Do not change node/edge order.
For each issue:
If type == "node": update the description and/or properties of the node with id.
If type == "edge": update the edge that matches the from and to with the correct properties.
Return only the corrected full JSON graph (same structure as input). Do not wrap it in any explanation.
...
def process_procedure(section_name):
    ...
    evaluation = read_text_file("v03-step4-evaluation.txt")
...
if procedural_info:
    save_to_json(procedural_info, "v03-step5-correct-usestep4e-improvedprompt.json")
else:

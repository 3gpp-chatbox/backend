...
def extract_procedural_info_from_text(section_name, text):
    prompt = f"""
    You are a 3GPP procedure extraction tool. Using your knowledge of 3GPP procedures but based on the provided text,Extract the **flow property details** of the procedure "{section_name}" while preserving the cause-effect structure.do not make assumption.


### **Extraction Rules:**  
🔹 **For each step:**  
   1️⃣ Identify **what action occurs** (description).  
   2️⃣ Identify **which entity performs it** (UE, AMF, etc.).  
   3️⃣ Identify **state changes** (if applicable).  
   4️⃣ Identify **messages sent/received** (and their consequences).  
   5️⃣ Identify **timers started/stopped** (and their purpose).  
   6️⃣ **Explicitly state how messages/timers relate to state changes.**  
...
{{
  ...
  "steps": [
    {{
      ...
      "entity": "UE or AMF",
      "state_change": "Old → New state (if applicable)",
      "messages": [
        {{
          "name": "Message X",
          "to": "UE or AMF",
          "triggers": ["State change Y", "Timer start Z"]
        }}
      ],
      "timers": [
        {{
...
if procedural_info:
    save_procedural_info_to_json(procedural_info, "step1-v3.json")
else:

...
1. **Descriptions**:  
   - For **state → event transitions**: Use the format:  
     `"[Key gating condition]"` (e.g., `"Lower layer indicates failure/release"`).  
   - For **event → state transitions**: Use the format:  
     `"[key Condition]; [key Action]"` (e.g., `"Registration attempt counter < 5 AND Not Emergency; Abort procedure, Stop T3510"`).  
...
2. **Conditions/Actions**:  
   ...
   - For event → state transitions, split into `[Condition]` and `[Action]` if both exist.
  ...
  "graph": {{
    ...
    "edges": [
      ...
      {{
        ...
        "type": "condition",
        "description": "Registration attempt counter < 5 AND Not Emergency; Abort procedure, Stop T3510"
      }}
    ]
  }}
}}

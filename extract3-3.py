...
def extract_procedural_info_from_text(text):
    ...
    prompt = f"""
  read below thunk text of a section carefully first:

    {text}

   remember, all you analysis and extract should be based on the thunk text, and you should not make any assumptions. now, list all procedures from the  section and then extract the the third procedure information from the list and use below mapping to extract the information(below is an example) finally,also return correct mermaid format and description for the third procedure.:
...
save_procedural_info_to_txt(procedural_info, "output3-3.txt")  # For plain text
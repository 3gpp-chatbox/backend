...
this secion is mainly about one procedure, analyze text  and fine that main procedure, and then focus on that procedure,extract the procedure information  and use below pattern to mapping it (below is an example) finally,also Return key information and description and  generate a  Flow Property Graph (FPG) visualization in a mermaid format suitable for web display. Ensure that:

The Flow Property Graph (FPG)structure accurately represents the process steps.
Nodes and connections follow a logical order.
Any conditional logic or loops are properly depicted. below is that example: 
...
# Process the section
procedural_info = process_section(614, 623, 'document_chunks.db')

# Print the extracted procedural info
# print("Extracted Procedural Information:\n")
# print(procedural_info)

save_procedural_info_to_txt(procedural_info, "procedure5-4-7-fpg.txt")  # For plain text
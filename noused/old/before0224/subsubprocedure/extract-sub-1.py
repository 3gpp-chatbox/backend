...
Now, analyze the text and find all sub-procedures. List them in the correct order, excluding any "General" sections or unrelated procedures. Then, focus on the first sub-procedure, extract its information, and map it using the following pattern (example provided below). Finally, return a Mermaid flowchart for the first sub-procedure using `graph TD` syntax (for a vertical layout). Ensure that all node labels are enclosed in double quotes and there are no extra spaces inside `{{}}` brackets.
 that example: 
...
save_procedural_info_to_txt(procedural_info, "sub-output-1.txt")  # For plain text
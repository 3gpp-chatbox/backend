...
class ActionType(str, Enum):
    ...
    N1N2_Message_Transfer = "N1N2MessageTransfer"
    NAS_Authentication_Request = "NAS Authentication Request"
    SMF_Context_Create = "SMF Context Create"
...
class Edge(BaseModel):
    ...
    properties: Optional[Dict[str, Any]] = Field(default_factory=dict)
    sub_graph: Optional["GraphModel"] = None  # 🔥 Recursive sub-procedure
...
class GraphModel(BaseModel):
    ...
    def validate_graph(cls, values):
        ...
        # ✅ Validate Edges Reference
        for edge in values.get("edges", []):
            if edge["from"] not in node_ids:
                raise ValueError(f"Invalid Edge: `from`: {edge['from']} not in nodes")
            if edge["to"] not in node_ids:
                raise ValueError(f"Invalid Edge: `to`: {edge['to']} not in nodes")

            # 🔥 Recursive Validation for Sub-Procedure
            if edge.get("sub_graph"):
                sub_graph = GraphModel(**edge["sub_graph"])  # Automatically Validate Sub-Procedures
...
def find_section_with_procedure_info(procedure_query):
    ...
    prompt = f"""
    you are 3gpp expert and 3GPP NAS specification procedures expert.
    Here are the NAS specification document section names and IDs:

    {sections_text}

     Please list the sections containing procedures.
    """

...
conn = sqlite3.connect('section_content_multiple_paragraphs.db')
cursor = conn.cursor()
...
paragraphs_per_chunk = 10 #how many paragraphs per chunk.
        ...
        if current_section_id and current_content:
            ...
            for paragraph in paragraphs:
                ...
                if len(current_chunk) == paragraphs_per_chunk:
                    chunks.append("\n\n".join(current_chunk)) #join the paragraphs.
                    current_chunk = []
            if current_chunk: #handle remaining paragraphs.
                chunks.append("\n\n".join(current_chunk))

            for chunk_id, chunk in enumerate(chunks):
                if chunk.strip():
                    # Get parent info from section table
                    cursor.execute("SELECT parent_section_id, parent_section_name FROM section WHERE section_id = ?", (current_section_id,))
                    ...
                    cursor.execute("INSERT INTO content (section_id, section_name, section_level, parent_section_id, parent_section_name, chunk_id, content_chunk) VALUES (?, ?, ?, ?, ?, ?, ?)",
                                   (current_section_id, current_section_name, current_section_level, parent_id, parent_name, chunk_id, chunk.strip()))
            current_content = []
...
print("Sections and content chunks with multiple paragraphs stored in the database.")
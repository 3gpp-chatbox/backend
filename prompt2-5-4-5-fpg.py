...
def extract_procedural_info_from_text(text):
    ...
    If the text contains a procedure in Mermaid code, please correct any mistakes in the Mermaid code and return only the corrected Mermaid code—no other words. 
    If the Mermaid content is incorrect according to the text description, correct it first, then return only the correct Mermaid code.Ensure that All node labels are enclosed in double quotes .There are no extra spaces inside {{}} brackets.
    remember remove ```mermaid and ```  in your response,no need any style in your response
    """
...
procedural_info = process_document("procedure5-4-5-fpg.txt")

# Save the extracted procedural info
save_procedural_info_to_mmd(procedural_info, "procedure5-4-5-fpg.mmd")

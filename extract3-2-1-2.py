...
 remember, all you analysis and extract should be based on the thunk text, and you should not make any assumptions. now, list all procedures from the  section and then focus one the second procedure ,extract the second procedure information  and use below pattern to mapping it (below is an example) finally,also Return description  a Mermaid flowchart for the second procedure using graph TD syntax (for a vertical layout). Ensure that All node labels are enclosed in double quotes ("").There are no extra spaces inside {{}} brackets. below is that example: 
...
save_procedural_info_to_txt(procedural_info, "output3-2-1-2.txt")  # For plain text